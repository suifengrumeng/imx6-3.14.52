	...
	case SND_SOC_BIAS_PREPARE:
		if (dapm->bias_level == SND_SOC_BIAS_STANDBY) {
			...
			ret = snd_soc_dai_set_clkdiv(codec_dai,
					WM8960_SYSCLKDIV, WM8960_SYSCLK_DIV_2);
			if (ret < 0) {
...
static ssize_t show_mic(struct device_driver *dev, char *buf)
{
	struct imx_priv *priv = &card_priv;
	int mic_status;
printk("11111111111111111111111111111111111111111111111111111111111111111111\n");
	if (!gpio_is_valid(priv->mic_gpio)) {
		strcpy(buf, "no detect gpio connected\n");
		return strlen(buf);
	}
	...
	{
		printk("22222222222222222222222222222222222222222222222222222222222222\n");
		strcpy(buf, "dmic\n");
	}
	return strlen(buf);
}
