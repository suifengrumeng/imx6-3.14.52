			    ...
			    struct snd_soc_dai *dai)
{
	...
	/* bit size: Audio Data Word Length*/
	/*
		Register Address: R7(07h) Digital Audio Interface Format(数字音频接口模式)
		BIT		Label		Default		Description
		 8 	   ALRSWAP		  0			Left/Right ADC chanel swap(左/右ADC通道交换)
										1 = Swap left and right ADC data in audio interface
										0 = Ouput left and right data as normal.

		 7	   BCLKINV		  0			BCLK invert bit(for master and slave modes)
										0 = BCLK not inverted
										1 = BCLK inverted

		 6		 MS			  0			Master/Slave Mode Control(主/从机模式控制)
										0 = Enable slave mode
										1 = Enable master mode

		 5	   DLRSWAP		  0			Left/Right DAC Chanel Swap(左/右DAC通道交换)
										0 = Output left and right data as normal
										1 = Swap left and right DAC data in audio interface
		
		 4	    LRP			  0			Right,left and I2S modes-LRCLK polarity(右,左和I2S模式-LRCLK极性)
										0 = normal LRCLK polarity
										1 = invert LRCLK polarity
										DSP Mode - mode A/B select(DSP模式-A/B模式选择)
										0 = MSB is available on 2nd BCLK rising edge after LRC rising edge(mode A)
										1 = MSB is availabel on 1st BCLK rising edge after LRC rising edge(mode B)

		3:2	  WL[1:0]		  10		Audio Data Word length(音频数据字长度)	 
										00 = 16 bits	01 = 20 bits	10 = 24 bits 11 = 32 bits(see note)

		1:0	  FORMAT[1:0]	  10		Aduio Data Format Select(音频数据模式选择)
										00 = Right justified	01 = Left justified		10 = I2S Format	11 = DSP Mode
